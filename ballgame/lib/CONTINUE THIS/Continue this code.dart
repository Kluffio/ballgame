...
class _GameScreenState extends State<GameScreen> with TickerProviderStateMixin {
  // Позиция шарика
  double ballX = 0.0;
  double ballY = -0.2; // Позиционируем пол шарика выше
  ...
  double jumpForce = -0.04;
  double diagonalForce = 0.025;
  ...
  void _addObstacle() {
    ...
    // Для уровней с 3 по 5 - меньше движущихся платформ
    if (widget.level.hasMovingPlatforms) {
      if (widget.level.league == MainMenu.leagues[0]) {
        // Зеленая лига (уровни 3-5)
        isMovingPlatform = random.nextDouble() > 0.7;
      } else {
        // Красная лига (уровни 6-10) - больше движущихся платформ
        isMovingPlatform = random.nextDouble() > 0.4;
      }
    }

    if (isMovingPlatform) {
      // Более быстрые и с большей амплитудой для красной лиги
      if (widget.level.league == MainMenu.leagues[1]) {
        speedX = 0.015 + random.nextDouble() * 0.025;
        amplitude = 0.4 + random.nextDouble() * 0.5;
      } else {
  ...
  void _resetGame() {
    setState(() {
      ballX = 0.0;
      ballY = -0.2; // Удерживаем шарик выше
      ballSpeedX = 0.0;
      ballSpeedY = 0.0;
  ...
  void _goToNextLevel(BuildContext context) {
    ...
    // Проверить, есть ли следующий уровень И
    // он принадлежит той же лиге (добавленное условие)
    if (currentIndex < MainMenu.levels.length - 1 &&
        UserProgress.unlockedLevels[currentIndex + 1] &&
        MainMenu.levels[currentIndex + 1].league == widget.level.league) {
      ...
      // Это был последний уровень лиги или следующий заблокирован, возвращаемся в меню
